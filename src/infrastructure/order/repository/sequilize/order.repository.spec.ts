...
describe("Order repository test", () => {
  ...
  it("should update a order", async () => {
    ...
    const orderItemAlt = new OrderItem(
      "1",
      product.name,
      25,
      product.id,
    ...
    const orderAlt = new Order("123", "123", [orderItemAlt]);

    await orderRepository.update(orderAlt);

    const orderModel = await OrderModel.findOne({
      where: { id: orderAlt.id },
      include: ["items"],
    });
